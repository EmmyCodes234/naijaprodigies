  ...
  return (
    <section className="bg-[#f2f0e9] py-24 lg:py-32 px-6 relative z-40 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
               <div 
                  className={`bg-white p-6 rounded-xl shadow-[4px_4px_0px_0px_rgba(15,60,58,1)] border-2 border-nsp-teal transition-all duration-300 hover:-translate-y-2 hover:shadow-[8px_8px_0px_0px_rgba(15,60,58,1)] ${
                    textVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                  }`}
                  style={{ transitionDelay: '300ms' }}
               >
                  <Icon icon="ph:trophy-fill" className="text-nsp-orange mb-3" width="32" height="32" />
                  <h3 className="font-bold text-nsp-dark-teal text-lg">World Champions</h3>
                  <p className="text-sm text-gray-600 mt-2 leading-snug">Home to the first African World Scrabble Champion.</p>
               </div>
               <div 
                  className={`bg-white p-6 rounded-xl shadow-[4px_4px_0px_0px_rgba(217,58,38,1)] border-2 border-nsp-red transition-all duration-300 hover:-translate-y-2 hover:shadow-[8px_8px_0px_0px_rgba(217,58,38,1)] ${
                    textVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                  }`}
                  style={{ transitionDelay: '500ms' }}
               >
                  <Icon icon="ph:brain-fill" className="text-nsp-red mb-3" width="32" height="32" />
                  <h3 className="font-bold text-nsp-dark-teal text-lg">Strategic Minds</h3>
                  <p className="text-sm text-gray-600 mt-2 leading-snug">Fostering critical thinking and superior vocabulary.</p>
               </div>
            </div>
          </div>
          ...
          <div 
            ref={imageRef as any}
            className={`relative will-change-transform transition-all duration-1000 ease-out delay-200 ${
        ...
        >
            {[
              { label: 'Club Members', value: '5,000+', icon: 'ph:users-three-fill' },
              { label: 'Tournaments', value: '120+', icon: 'ph:star-fill' },
              { label: 'Global Rank', value: '#1', icon: 'ph:trophy-fill' },
              { label: 'Prodigies', value: '500+', icon: 'ph:brain-fill' },
            ].map((stat, idx) => (
              <div 
                key={idx} 
